  ...
  - *Post-build Actions*  
    - Send build artifacts over SSH  
      - *SSH Publishers*  
	       - Exec command: 
                ```sh 
                ansible-playbook -i /opt/docker/hosts /opt/docker/kubernetes-sample-deployment.yml;
                ansible-playbook -i /opt/docker/hosts /opt/docker/kubernetes-sample-service.yml;
                ```
Save and run the job.
